...
if _%1_==_-i_ set TCCDIR=%2&& goto :a2
if _%1_==_-b_ set BINDIR=%2&& goto :a2
...
set ARG=%1
set ARG=%ARG:.dll=.lib%
if _%1_==_-shared_ set ARG=-LD
if _%1_==_-o_ shift && set ARG=-Fe%2
set CMD=%CMD% %ARG%
shift
if not _%1_==__ goto :c0
echo on
...
if _%BINDIR%_==__ set BINDIR=%TCCDIR%
...
echo>>..\config.h #endif
if not _%GITHASH%_==__  (echo>>  ..\config.h #define TCC_GITHASH "%GITHASH%")
...
:compiler_done
@if _%EXES_ONLY%_==_yes_ goto :files_done
...
:tcc-doc.html
@if not _%DOC%_==_yes_ goto :doc-done
